...
// Function to set up UART for communication
void setup() {
    UBRR0H = 0;
...
uint32_t measurePulse() {
    ...
    // Wait for rising edge on ECHO_PIN
    while (!(PIND & (1 << ECHO_PIN)));

    time_start = TCNT1;  // Store start time

    // Wait for falling edge on ECHO_PIN
    while (PIND & (1 << ECHO_PIN));

    time_end = TCNT1;  // Store end time
    ...
    // Convert pulse duration to microseconds
...
uint32_t getDistance() {
    ...
    // Take multiple readings to get an average
    for (uint32_t i = 0; i < measurements; i++) {
        sendTriggerPulse();

        uint32_t duration = measurePulse();

        // Convert duration to distance in cm (Speed of sound is 0.0343 cm/us)
        uint32_t distance = (duration * 0.0343) / 2;

        totalDistance += distance;
        _delay_ms(100);  // Small delay between measurements
    }
...
void controlLED(uint32_t distance) {
    ...
    // Flash PB5 (Yellow LED "L") when the object is outside [D1, D2]
